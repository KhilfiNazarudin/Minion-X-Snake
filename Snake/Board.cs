...
namespace Snake
{
    ...
    {
        int maxRow; //= 10;
        int maxCol; //= 20;       //Max 10 rows, and 20 columns in the board
        int squareSize = 40;                //Each square is 30px by 30px

        PictureBox[,] squares;

        public Board(Form mainForm,int rows,int cols)
        {
            ...
            for (int row = 0; row < maxRow; row++)
            {
                for (int col = 0; col < maxCol; col++)
                {
                    ...
                    squares[row, col].BackColor = Color.Yellow;
                    squares[row, col].Image = Properties.Resources.banana;
